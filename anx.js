...
function ANXClient(key, secret, currency) {
  ...
  self.info = function(callback) {
    makeRequest("/api/2/"+self._currency + "/money/info", {}, callback);
  };

  self.idKey = function(callback) {
    makeRequest("/api/2/"+self._currency + "/money/idkey", {}, callback);
  };

  self.orders = function(callback) {
    makeRequest("/api/2/"+self._currency + "/money/orders", {}, callback);
  };

  self.currency = function(callback) {
    makePublicRequest("/api/2/"+self._currency + "/money/currency", {}, callback);
  };

  self.ticker = function(callback) {
    makePublicRequest("/api/2/"+self._currency + "/money/ticker", {}, callback);
  };

  self.tickerFast = function(callback) {
    makePublicRequest("/api/2/"+self._currency + "/money/ticker_fast", {}, callback);
  };

  self.quote = function(type, amount, callback) {
    makePublicRequest("/api/2/"+self._currency + "/money/order/quote", {
      "type": type,
      "amount": amount
    }, callback);
  };
  ...
  self.add = function(type, amount_int, price_int, callback) {
    ...
    makeRequest("/api/2/"+self._currency + "/money/order/add", args, callback);
  };

  self.cancel = function(id, callback) {
    makeRequest("/api/2/"+self._currency + "/money/order/cancel", {
      "oid": id
    }, callback);
  };
  ...
  self.result = function(type, order, callback) {
    makeRequest("/api/2/"+self._currency + "/money/order/result", {
      "type": type,
      "order": order
    }, callback);
  };
  ...
  self.lag = function(callback) {
    makePublicRequest("/api/2/"+self._currency + "/money/order/lag", {}, callback);
  };

  self.fetchTrades = function(since, callback) {
    var args = {};
    if (typeof since != undefined) args.since = since;
    return makePublicRequest("/api/2/"+self._currency + "/money/trades/fetch", args, callback);
  };

  self.fetchDepth = function(callback) {
    makePublicRequest("/api/2/"+self._currency + "/money/depth/fetch", {}, callback);
  };

  self.fullDepth = function(callback) {
    makePublicRequest("/api/2/"+self._currency + "/money/depth/full", {}, callback);
  };
  ...
  self.depositAddress = function(callback) {
    self.info(function(err, json) {
      ...
      makeRequest("/api/2/"+self._currency + "/money/bitcoin/get_address", args, callback);
    });
  }

  self.dataToken= function(callback) {
    makeRequest("/api/3/dataToken", {}, callback);
  };
